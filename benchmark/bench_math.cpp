...
static void BM_Hamu_Add(benchmark::State& state) {
    static const auto va = generate_vecs<hamu::float4>(BATCH_SIZE);
    static const auto vb = generate_vecs<hamu::float4>(BATCH_SIZE);
    for (auto _ : state) {
        hamu::float4 sink{0};
